/**
 * staleBurstReaper 单测：超时选择 + peek 跳过 + kill + ABORTED 迁移 + action-log。
 * ADL: doc/structurizr/STRATEGY-PLANNING-LAYER.md §9
 */
...
import { DEFAULT_STALE_AWAITING_POLICY } from './strategy-types.js';
...
describe('selectNeedsReview', () => {
  it('超 requireProgressSignalAfterMs 但未到硬上限 → 复审', () => {
    const t = task({ lastTickAt: new Date(NOW - 4 * DAY).toISOString() }); // 4d ∈ (3d,7d]
