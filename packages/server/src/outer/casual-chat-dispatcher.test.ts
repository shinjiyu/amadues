...
describe('casual-chat-dispatcher', () => {
  ...
  it('busy verdict → no dispatch', async () => {
    ...
    const deps: CasualChatDispatchDeps = {
      ...
      kpiRegistry,
      imClient: null,
      toolCtx: {} as CasualChatDispatchDeps['toolCtx'],
      getLlmEnv: () => null,
    };
    const result = await dispatchCasualChat(deps, baseSnapshot(), {
      level: 'busy',
  ...
  it('has active KPI → defers casual_chat（KPI 由 kpiManager 派）', async () => {
    const root = createTestDataRoot('casual-kpi-');
    patchAutonomyPolicy(root.dataRoot, { enabled: true });
    const registry = new InnerBrainRegistry(root.dataRoot);
    const kpiRegistry = new KpiRegistry(root.dataRoot);
    kpiRegistry.create({ description: '测试 KPI', createdBy: 'test' });

    const setGoalSpy = vi.spyOn(outerTools, 'executeOuterTool');
    ...
    const deps: CasualChatDispatchDeps = {
      ...
      toolCtx: {
        ...
        kpiRegistry,
      } as CasualChatDispatchDeps['toolCtx'],
      getLlmEnv: () => null,
    };

    const result = await dispatchCasualChat(deps, baseSnapshot(), idleVerdict());
    expect(result.dispatched).toBe(false);
    expect(result.reason).toBe('casual_chat_defer_to_kpi');
    expect(setGoalSpy).not.toHaveBeenCalled();
    root.cleanup();
  });

  it('KPI sprint 在途 → 跳过 casual_chat', async () => {
    const root = createTestDataRoot('casual-kpi-dup-');
    patchAutonomyPolicy(root.dataRoot, { enabled: true });
    const registry = new InnerBrainRegistry(root.dataRoot);
    const kpiRegistry = new KpiRegistry(root.dataRoot);
    ...
    const result = await dispatchCasualChat(deps, baseSnapshot(), idleVerdict());
    expect(result.dispatched).toBe(false);
    expect(result.reason).toBe('kpi_sprint_in_progress');
    expect(setGoalSpy).not.toHaveBeenCalled();
    root.cleanup();
  });

  it('no KPI + high chat probability → may dispatch casual_chat', async () => {
    const root = createTestDataRoot('casual-chat-');
    patchAutonomyPolicy(root.dataRoot, { enabled: true });
