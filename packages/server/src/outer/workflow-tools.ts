    ...
    function: {
      ...
      parameters: {
        ...
        properties: {
          ...
          steps_json: {
            type: 'string',
            description: 'WorkflowStep[] JSON：[{id,action,args?,expect:{fileExists|exitCode|stdoutContains}}]',
          },
          playbook_path: {
            type: 'string',
  ...
  try {
    switch (name) {
      ...
      case 'workflow_promote': {
        ...
        const wf = promoteWorkflow(store, input, { drive9: ctx.workflowDrive9Store });
        return {
          replied: false,
