...
export function createPromoteExecutableWorkflowTool(
  ...
  return {
    ...
    description:
      ...
      '仅在路径已稳定、下次必须照做时调用；知识用 record_fact，临场战术用 LocalNode/record_skill。' +
      'from=auto 时扫描 local_dag / playbook；也可传 steps_json。',
    parameters: {
      workflow_id: {
        type: 'string',
      ...
      from: {
        type: 'string',
        description: 'auto（扫 workspace 产物）| steps（用 steps_json）',
      },
      steps_json: {
        type: 'string',
        description: 'from=steps 时：WorkflowStep[] JSON',
      },
      tags: {
        type: 'string',
    ...
    async call(args) {
      ...
      try {
        ...
        const wf = promoteWorkflow(store, draft);
        return {
          ok: true,
